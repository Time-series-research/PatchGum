...
def TRADE(args, pred, true):
    ...
    y_step_1 = torch.cat((torch.zeros(8, 1, 1).cuda(), y_mid), 1)

    x_mid = true.clone()
    x_mid = x_mid[:, torch.arange(x_mid.size(1)) != pre_length - 1, :]
    x_step_1 = torch.cat((torch.zeros(8, 1, 1).cuda(), x_mid), 1)

    zero_matrix = torch.zeros(8, pre_length, 1).cuda()
